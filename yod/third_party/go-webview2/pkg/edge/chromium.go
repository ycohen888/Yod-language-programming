...
func (e *Chromium) MessageReceived(sender *ICoreWebView2, args *iCoreWebView2WebMessageReceivedEventArgs) uintptr {
	...
	text := w32.Utf16PtrToString(message)
	if message != nil {
	...
	if text == "" {
		...
		_, _, _ = args.vtbl.GetWebMessageAsJSON.Call(
			uintptr(unsafe.Pointer(args)),
			uintptr(unsafe.Pointer(&jsonMsg)),
		)
		text = w32.Utf16PtrToString(jsonMsg)
		if jsonMsg != nil {
			windows.CoTaskMemFree(unsafe.Pointer(jsonMsg))
		}
		fromJSON = true
	}
	// postMessage(string) → GetWebMessageAsJSON מחזיר JSON של מחרוזת (עם מרכאות) — לפרוס פעם
	if fromJSON && len(text) >= 2 && text[0] == '"' {
		var inner string
		if err := json.Unmarshal([]byte(text), &inner); err == nil && inner != "" {
			text = inner
		}
	}
	if e.MessageCallback != nil && text != "" {
...
func (e *Chromium) AddWebResourceRequestedFilter(filter string, ctx COREWEBVIEW2_WEB_RESOURCE_CONTEXT) {
	...
	if err != nil {
		log.Printf("AddWebResourceRequestedFilter: %v", err)
	}
}
...
// SetDefaultBackgroundColor מגדיר את רקע WebView2 לפני טעינת דף (מונע הבזק לבן).
func (e *Chromium) SetDefaultBackgroundColor(a, r, g, b uint8) error {
	if e.controller == nil {
		return nil
	}
